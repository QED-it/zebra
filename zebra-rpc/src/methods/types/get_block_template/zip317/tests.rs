...
fn excludes_tx_with_unselected_dependencies() {
    ...
    // FIXME: is this a correct change (try_from_raw_transparent_p2pkh to try_from_transparent_p2pkh)?
    let miner_address =
        Address::try_from_transparent_p2pkh(NetworkKind::from(network.clone()).into(), [0x7e; 20])
