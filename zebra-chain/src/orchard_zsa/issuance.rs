...
impl ZcashDeserialize for Nullifier {
    fn zcash_deserialize<R: io::Read>(mut reader: R) -> Result<Self, SerializationError> {
        Option::from(Nullifier::from_bytes(&reader.read_32_bytes()?))
            .ok_or_else(|| SerializationError::Parse("Invalid orchard_zsa Nullifier!"))
    }
}
...
fn zcash_deserialize_random_seed<R: io::Read>(
    ...
    Option::from(RandomSeed::from_bytes(reader.read_32_bytes()?, nullifier))
