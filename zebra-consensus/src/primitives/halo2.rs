...
lazy_static::lazy_static! {
    ...
    pub static ref VERIFYING_KEY_PRE_NU6_2: ItemVerifyingKey =
        ItemVerifyingKey::build_for_version::<OrchardVanilla>(OrchardCircuitVersion::InsecurePreNu6_2);
...
#[cfg(all(zcash_unstable = "nu7", feature = "tx_v6"))]
lazy_static::lazy_static! {
    /// The Orchard Action verifying key for the ZSA circuit.
...
impl RequestWeight for Item {
    fn request_weight(&self) -> usize {
        match &self.bundle {
            OrchardBundle::OrchardVanilla(bundle) => bundle.actions().len(),
            #[cfg(all(zcash_unstable = "nu7", feature = "tx_v6"))]
            OrchardBundle::OrchardZSA(bundle) => bundle.actions().len(),
        }
    }
}
...
/// Global batch verification context for ZSA Halo2 Action proofs.
#[cfg(all(zcash_unstable = "nu7", feature = "tx_v6"))]
