...
Rust ≥ 1.87. Done, the indexer syncs this chain and the explorer shows its
blocks. ZSA burn and issuance are still unindexed — the Ironwood columns are
zeroed, not remapped.

It also needs the node running with persistent state. On this box that comes
from `/opt/zebra/docker-compose.override.yml`, not from this file — `ops.sh
deploy-files` overwrites `docker-compose.yml` from the branch but leaves the
override alone.
